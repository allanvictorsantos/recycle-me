...
import React from 'react'; // Garante que React está no escopo para useRef

function App() {
  // --- PASSO 2: PEGAR O ESTADO DE AUTENTICAÇÃO E A FUNÇÃO LOGOUT ---
  const { isAuthenticated, logout } = useAuth();

  // --- O RESTO DOS SEUS ESTADOS (mantido igual) ---
  ...
  // --- EFEITOS COLATERAIS (lógica mantida) ---
  useEffect(() => {
    const root = window.document.documentElement;
    // Função para verificar e aplicar o tema
    const checkTheme = () => {
      const storedTheme = localStorage.getItem('theme') || 'auto'; // Lê o tema guardado ou 'auto'
      const isDark = storedTheme === 'dark' || (storedTheme === 'auto' && window.matchMedia('(prefers-color-scheme: dark)').matches);
      root.classList.toggle('dark', isDark);
    };
    checkTheme(); // Aplica na carga

    // Ouve mudanças do sistema operacional (se o tema for 'auto')
    const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');
    mediaQuery.addEventListener('change', checkTheme);

    return () => mediaQuery.removeEventListener('change', checkTheme);
  }, [theme]); // Dependência [theme] mantida
  ...
  useEffect(() => {
    ...
    // Adiciona o listener apenas se o painel estiver aberto
    if (isAccessibilityPanelOpen) {
      document.addEventListener("mousedown", handleClickOutside);
    }
    // Função de limpeza remove o listener
    return () => {
      document.removeEventListener("mousedown", handleClickOutside);
    };
  }, [isAccessibilityPanelOpen]); // Dependência correta

  // --- Novas Funções Helper para Menu Mobile (mantidas) ---
  ...
  return (
    <>
      ...
      <div id="page-wrapper" className={`min-h-screen flex flex-col bg-brand-cream dark:bg-brand-dark text-brand-dark dark:text-gray-300 transition duration-300 ${isReadingMode ? 'reading-mode' : ''}`}>
        ...
        <header className="header-gradient dark:bg-gray-800 shadow-sm sticky top-0 z-50 dark:border-b dark:border-gray-700">
          <div className="container mx-auto flex justify-between items-center p-4">
            ...
            {/* --- NAVEGAÇÃO DESKTOP ATUALIZADA --- */}
            <nav id="main-nav" className="hidden md:flex items-center gap-6 lg:gap-8">
              {/* Links Públicos (com 'to' corrigido e 'smooth' ADICIONADO) */}
            ...
            <div className="flex items-center gap-4">
              {/* --- MUDANÇA: Ícone de Perfil (sempre aponta para /perfil) --- */}
              <Link to="/perfil" title={isAuthenticated ? "Acessar perfil" : "Acessar conta"} className="text-2xl hover:text-brand-text-green transition-colors dark:text-gray-300 dark:hover:text-brand-green">
          ...
          <div id="mobile-menu" className={`md:hidden bg-brand-cream dark:bg-gray-800 pb-4 ${isMobileMenuOpen ? '' : 'hidden'}`}>
            {/* Links Públicos (com 'smooth' ADICIONADO e 'onClick' REMOVIDO) */}
            {/* O HashLink não aceita onClick, mas o clique nele deve fechar o menu */}
            {/* Vamos envolver em um div que fecha o menu */}
            ...
            {/* Links normais mantêm o onClick para fechar o menu */}
            <Link to="/mapa" className="block text-center py-2 px-4 text-sm font-bold uppercase text-brand-dark dark:text-gray-200 hover:bg-brand-light-green dark:hover:bg-gray-700" onClick={closeMobileMenu}>Mapa</Link>
            ...
            {!isAuthenticated ? (
              ...
              <>
                <Link to="/perfil" className="block text-center py-2 px-4 text-sm font-bold uppercase text-brand-dark dark:text-gray-200 hover:bg-brand-light-green dark:hover:bg-gray-700" onClick={closeMobileMenu}>Meu Perfil</Link>
        ...
        {/* Footer Completo (com 'smooth' ADICIONADO) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
              ...
              <div> <h3 className="text-lg font-bold mb-4">Links Rápidos</h3>
                <ul className="space-y-2 text-sm">
                  ...
                  <li><Link to="/cadastro" className="text-gray-400 hover:text-white transition-colors">Seja um Parceiro</Link></li>                   <li><a href="#" className="text-gray-400 hover:text-white transition-colors">Contato</a></li>
                </ul>
              </div>
      ...
      {/* Botões Flutuantes Completos */}
      <button id="back-to-top-button" onClick={scrollToTop} title="Voltar ao topo" aria-label="Voltar ao topo" className={`fixed bottom-20 right-5 z-[60] w-12 h-12 md:w-14 md:h-14 bg-brand-dark/80 backdrop-blur-sm text-white rounded-full shadow-lg flex items-center justify-center text-xl hover:bg-gray-700 transition-all duration-300 ${isBackToTopVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}> <i className="fas fa-arrow-up"></i> </button>
      <button id="accessibility-button" onClick={toggleAccessibilityPanel} title="Abrir menu de acessibilidade" aria-label="Abrir menu de acessibilidade" aria-haspopup="true" aria-expanded={isAccessibilityPanelOpen} className="fixed bottom-5 right-5 z-[60] w-12 h-12 md:w-14 md:h-14 bg-brand-dark text-white rounded-full shadow-lg flex items-center justify-center text-2xl hover:bg-gray-700 transition-colors"> <i className="fas fa-universal-access"></i> </button>
      ...
      <div
        ...
        className={`fixed bottom-20 md:bottom-24 right-5 z-[60] bg-white/90 backdrop-blur-md dark:bg-gray-800/90 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 w-64 overflow-hidden transition-all duration-300 ${isAccessibilityPanelOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}
      >

        {/* Conteúdo completo do painel de acessibilidade */}
        <div className="flex justify-between items-center p-3 border-b border-gray-200 dark:border-gray-700">
          <h3 id="accessibility-panel-title" className="font-bold text-md text-brand-dark dark:text-white">Acessibilidade</h3>
          <button onClick={toggleAccessibilityPanel} title="Fechar menu" aria-label="Fechar menu de acessibilidade" className="text-gray-500 hover:text-brand-dark dark:hover:text-white"><i className="fas fa-times"></i></button>
        </div>
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center justify-around p-1 bg-gray-100 dark:bg-gray-900 rounded-lg">
            <button onClick={() => changeTheme('light')} title="Modo Claro" aria-pressed={theme === 'light'} className={`p-2 rounded-md ${theme === 'light' ? 'bg-brand-green text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
              <i className="fas fa-sun"></i>
            </button>
            <button onClick={() => changeTheme('auto')} title="Automático (Sistema)" aria-pressed={theme === 'auto'} className={`p-2 rounded-md ${theme === 'auto' ? 'bg-brand-green text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
              <i className="fas fa-desktop"></i>
            </button>
            <button onClick={() => changeTheme('dark')} title="Modo Escuro" aria-pressed={theme === 'dark'} className={`p-2 rounded-md ${theme === 'dark' ? 'bg-brand-green text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
